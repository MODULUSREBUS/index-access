...
async fn can_call_new() {
    ...
    let _storage = IndexAccessFs::new(dir.path())
        .await.unwrap();
}
...
async fn can_write() {
    let dir = Builder::new().prefix("test_iaf").tempdir().unwrap();
    let mut storage = IndexAccessFs::new(dir.path()).await.unwrap();
    storage.write(String::from("write1"), b"hello").await.unwrap();
    storage.write(String::from("write2"), b"world").await.unwrap();
}
...
async fn can_read() {
    let dir = Builder::new().prefix("test_iaf").tempdir().unwrap();
    let mut storage = IndexAccessFs::new(dir.path()).await.unwrap();
    storage.write(String::from("read"), b"hello world").await.unwrap();
